...
void main(string[] args)
{
	...
	version (Windows) enum ext = ".exe";
	else              enum ext = "";
	auto expected = [
		"copyfiles-test" ~ ext, "file_to_copy.txt", "file_to_copy_mask1.txt",
	...
	].map!(a=>a.replace("/", dirSeparator));

	assert(files.equal(expected), files.join(", "));
}
